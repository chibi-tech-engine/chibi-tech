...
#include "allocators/dynamic-allocator.h"
...
typedef struct memory_system_state memory_system_state;
...
CT_API void*
ct_allocate(u64 size, memory_tag tag)
{
    return ct_allocate_aligned(size, 1, tag);
}

CT_API void*
ct_allocate_aligned(u64 size, u16 alignment, memory_tag tag)
{
    CT_ASSERT(tag < memory_tag_max);
...
CT_API void
ct_free(void* ptr, memory_tag tag)
{
    CT_ASSERT(tag < memory_tag_max);
    if (ptr) {
...
CT_API void*
ct_reallocate(void* block, u64 old_size, u64 new_size, memory_tag tag)
{
    return ct_reallocate_aligned(block, old_size, new_size, 1, tag);
}

CT_API void*
ct_reallocate_aligned(void* block, u64 old_size, u64 new_size, u16 alignment, memory_tag tag)
{

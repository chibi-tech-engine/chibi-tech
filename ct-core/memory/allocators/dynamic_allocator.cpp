...
//
static bool merge_block_left(block_header *left_header, block_header* right_header)
{
...
void dynamic_allocator_initialize(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator** out)
{
    ...
    *memory_requirement = total_size + sizeof(dynamic_allocator);
    ...
    if (!memory)
    { //just fetching the system size, early out
        return;
    }
...
void* dynamic_allocator_allocate(dynamic_allocator* self, u64 size)
{
    ...
    if (real_size > self->free_memory)
    { //not enough memory
        return nullptr;
    }

    if (!self->free_blocks)
    { //there is enough memory, but allocations are too fragmented.
        return nullptr;
    }
    ...
    if (!iter)
    { //couldn't find a block of memory to satisfy the user's request
        return nullptr;
    }
    ...
    //split the block if possible, don't want to return a large allocation if the request was much smaller
    {
        ...
        if (leftover_size >= c_min_allocation_size)
        { //the leftover space is large enough to split the block.
...
void dynamic_allocator_free(dynamic_allocator* self, void* ptr)
{
    ...
    else
    {
        ...
        {
            if ((uintptr_t)block < (uintptr_t)iter->next_ptr)
            { // found a block the allocation will fit into
                break;
            }
            iter = iter->next_ptr;
